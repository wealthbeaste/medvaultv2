...
module.exports = function registerRoutes(app) {
  ...
  const { query, pool, getNextReceiptNumber } = require('../database/db');
...
app.post('/api/inventory', auth, async (req, res) => {
  ...
  // ── Idempotency guard (outside transaction — read-only check) ────────────
  ...
  // ── STEP 1 FIX: Wrap drug + batch insert in one transaction ─────────────
  // Without this, a crash between the two INSERTs leaves an orphan drug row
  // with no batch, or worse, a batch pointing at a non-existent drug.
  ...
  try {
    ...
    const drugResult = await client.query(
      `INSERT INTO drugs (
        pharmacy_id, name, generic_name, category, quantity,
        unit_price, cost_price, threshold, barcode, sku, supplier, expiry_date
  ...
  app.post('/api/sales', auth, async (req, res) => {
    ...
    // between inserting the sale and deducting stock, inventory would be wrong.
    // Now: receipt generation + sale insert + items + stock deduction are one
    // atomic operation. If anything fails, the entire sale is rolled back.
    const client = await pool.connect();
    try {
      await client.query('BEGIN');

      // Atomic receipt number (STEP 2 FIX — no more COUNT(*) race condition)
      ...
      for (const item of items) {
        await client.query(
          `INSERT INTO sale_items (sale_id,drug_id,drug_name,quantity,unit_price,total_price)
             VALUES ($1,$2,$3,$4,$5,$6)`,
          [sale.id, item.drug_id||null, item.drug_name, item.quantity,
      ...
      res.status(500).json({ error:'Sale failed: '+e.message });
    } finally {
      client.release();
    }
  });
